<script>
	...
	import { DIFFICULTIES, DROPDOWN_DURATION, DIFFICULTY_CUSTOM, GAME_MODES } from '@sudoku/constants';
...
<div class="dropdown">
	<button class="dropdown-button" on:click={dropdownVisible ? hideDropdown : showDropdown} title="{dropdownVisible ? 'Close' : 'Open'} Menu" disabled={$gameMode === GAME_MODES.CREATE}>
		...
		<span class="text-2xl tracking-wider">{$gameMode === 'create' ? 'Creator Mode' : ($difficulty === DIFFICULTY_CUSTOM ? 'Custom' : DIFFICULTIES[$difficulty])}</span>
	</button>
	...
	{#if $difficulty === DIFFICULTY_CUSTOM && selectedExampleName}
