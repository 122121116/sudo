<script>
	...
	function updateUndoRedoState() {
		if (window.userGridHistory && window.userGridFuture) {
			canUndo = window.userGridHistory.length > 0;
			canRedo = window.userGridFuture.length > 0;
		}
	}

	// 监听userGrid变化，更新undo/redo状态
	onMount(() => {
		const unsubscribe = userGrid.subscribe(() => {
			if (typeof window !== 'undefined') {
				window.userGridHistory = window.userGridHistory || [];
				window.userGridFuture = window.userGridFuture || [];
				updateUndoRedoState();
			}
		});
		return unsubscribe;
